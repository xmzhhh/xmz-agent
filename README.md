...
- 当前基金路由集合在创建 Router 时加载到内存，默认演示代码为 017811；未来持仓持久化后
  将由数据库中的基金持仓自动生成，不通过六位代码格式猜测资产类型。
- `scripts/check_real_market_data.py` 可在 PyCharm 中同时完成基金和黄金的真实联网验收；
  pytest 使用假数据和假 HTTP 传输层，不消耗真实 API 额度。
- `scripts/check_market_data_routing.py` 可在 PyCharm 中离线展示一个 Service 如何把基金和黄金
