...
- 会话和完整 user/assistant/tool 消息保存在同一个本地 SQLite，应用重启后可以继续读取；
- Agent 通过三个只读工具查询组合快照、单项持仓和交易账本摘要，不能修改资产或执行交易；
- 上下文按“系统提示 → 已确认长期记忆 → 滚动摘要 → 近期原始消息”确定顺序组装；
- 模型只能生成待确认的结构化记忆候选，用户必须通过独立 API 确认、拒绝或删除；
- `/api/v1/agent/sessions` 提供会话管理，`/api/v1/memories` 提供候选、状态和审计管理；
- 未配置 `LLM_API_KEY` 时资产面板和会话/记忆管理仍可启动，只有 Agent chat 返回 503。

Jinja2 资产页面现已包含 AI 资产助手区域，可以新建/切换/删除会话、恢复完整消息与工具轨迹、
选择本轮相关资产范围，并查看、确认、拒绝或删除长期记忆。浏览器只负责渲染 API 结果，不在
前端执行 Agent 循环或金融计算，也没有任何聊天触发的持仓写入口。

网页聊天当前使用普通请求—响应模式：点击发送后，后端完成必要工具调用、最终回答、整轮消息
提交和候选抽取，再一次性返回结果。后续可增加 SSE 流式事件，但在此之前先保证失败不留下半轮
消息、刷新可恢复历史这一更重要的正确性边界。CLI 复用同一持久化 Agent 和最终阶段验收属于
Phase 8 后续小阶段。
...
适合只使用资产和记忆管理功能；要在网页中与 AI 对话，还必须在本地 `.env` 配置有效的
`LLM_API_KEY`。如果需要手机从可信局域网访问，可把形参改成
