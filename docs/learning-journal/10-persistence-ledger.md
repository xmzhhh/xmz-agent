...
相关 Issue / PR：Issue #15 / PR #16（草稿，待复核与合并）。
...
- 复核本阶段唯一 PR #16，将其标记为 Ready 后合并；随后同步 `main`、清理功能分支并发布
  `v0.3.0`。
...
- PyCharm 人工验收：2026-08-19 运行 Step 08，成功完成数据库迁移、两次应用重启、卖出试算与
  确认卖出；临时数据库自动删除、未访问真实行情，进程退出代码为 0。
