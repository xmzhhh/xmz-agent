...
- `uv run python -m pytest -q`：296 个测试通过；第四小阶段新增 7 个测试，相关会话与记忆目标
  测试合计 23 个通过。
- `uv run ruff check .`：通过。
- `uv run mypy`：112 个源文件通过严格类型检查。
...
把 `ContextAssembler` 接入持久化 Agent 编排：完整成功的一轮才原子保存 user、assistant 和 tool
消息；模型或工具失败不留下半截历史。之后再注册只读资产工具，共享 Phase 7 SQLite 事实来源。
